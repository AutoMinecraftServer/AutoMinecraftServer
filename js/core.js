...
$(document).on('contextmenu', e => {
    ...
    if ($(e.target).get(0).tagName === 'INPUT') {
        ...
        remote.Menu.buildFromTemplate([
            ...
            {type: 'separator'},
            {label: '「' + name + '」をBAN', click: () => send_command(id, 'ban ' + name)},
            {label: '「' + name + '」のIPをBAN', click: () => send_command(id, 'ban-ip ' + name)},
            {label: '「' + name + '」にOP権限を与える', click: () => send_command(id, 'op ' + name)},
            {label: '「' + name + '」にOP権限を剥奪', click: () => send_command(id, 'deop ' + name)},
            {type: 'separator'},
            {label: '「' + name + '」を退出させる', click: () => send_command(id, 'kick ' + name)},
            {label: '「' + name + '」をキル', click: () => send_command(id, 'kill ' + name)},
            {label: '「' + name + '」のアイテムを削除', click: () => send_command(id, 'clear ' + name)},
        ]).popup(remote.getCurrentWindow());
    } else if (b)
        remote.Menu.buildFromTemplate([
...
$(window).on('resize', () => {
    ...
    $('#main').height(h + 'px');
    Object.keys(profiles).forEach(k => $('#' + profiles[k].id + '_content').find('.dataTables_scrollBody').height(h - 265 + 'px'));
...
$('#min_memory_slider').on('input', function() {
...
$('.properties_text').change(function() {
    properties[$(this).attr('id')] = $(this).val();
    ipc.send('save_properties', { location: properties_location, data: properties });
});
$('.properties_drop').click(function() {
...
$('#eula_modal').on('hide.bs.modal', () => $('#eula_agree').off('click'));
...
function port_open(id, port_num) {
    upnp.findGateway((gateway_error, info, gateway_ip) => {
        ...
        if (id) port[id].gateway_ip = gateway_ip;
        var local_ip = [];
        var interfaces = require('os').networkInterfaces();
        for (var dev in interfaces)
            interfaces[dev].forEach(details => !details.internal && details.family === 'IPv4' ? local_ip.push(details.address)  : '');
        var main_local_ip = '';
        ...
        if (id) port[id].local_ip = main_local_ip;
...
function timer(id) {
    ...
    if (!(timer_count[id] / 60 / profiles[id].backup_minute).toString().includes('.') && profiles[id].backup === true) {
        backup_flag[id] = true;
...
function load_data(directory) {
    fs.readdir(directory, (err, folders) => {
        ...
        for (var num in folders) {
            ...
            else if (name === 'server.properties' || name === 'whitelist.json' || name === 'usercache.json' || name === 'ops.json' || name === 'banned-players.json' || name === 'banned-ips.json' || name.includes('.jar')) all++;
        }
...
function create_detail(extra) {
    ...
    function add_detail(i, e) {
        ...
        $('#' + e.id + '_cmd_input').keyup(e_ => e_.keyCode === 13 ? send_command(e.id) : '');
        ...
        .click(function(evt) {
            ...
            eout(() => $('.pop_button').text('アドレスをコピーする'), 2000);
        })
        .popover({
            title: function() {
        ...
        $('#' + e.id + '_cmd_input').autoComplete({
            ...
            source: function(term, suggest) {
                ...
                var matches = [];
                for (var i = 0; i < choices.length; i++) if (choices[i].toLowerCase().includes(term)) matches.push(choices[i]);
                suggest(matches);
            }
        });
...
function progress(id, jar, zip) {
    ...
    $('#progress_text').show();
    if (jar && zip === undefined) start_download(id, jar.ver, jar.type, jar.latest, 1);
    else if (jar === undefined && zip) start_unzip(id, zip.data, zip.base, zip.count, zip.type, 1);
    else if (jar && zip) start_unzip(id, zip.data, zip.base, zip.count, zip.type, 0.3, jar);
}
...
function start_unzip(id, file, base, data_count, type, per, extra) {
    ...
    ipc.once('unzip_finish', () => {
        percent(100 * per, '解凍完了(' + 100 * per + '%)');
        ipc.removeAllListeners('unzip_progress');
        if (extra) start_download(id, extra.ver, extra.type, extra.latest, 0.7);
          ...
          .then(res => res.text()).then(data => {
            ...
            .on('close', err => {
              ...
              e.stdout.on('data', data => {
                if (data.includes('you should now be able to run the file'))
                file = data.slice(data.indexOf('file') + 5, data.indexOf('.jar') + 4);
                c++;
                percent(p(Math.round(15 + c / 1.5)), 'インストール中...(' + p(Math.round(15 + c / 1.5)) + '%)');
              });
              //e.stderr.on('data', function(data) { console.log(data); })
              e.on('exit', () => {
...
function save_indices(id, text) {
    if (indices[id].includes(text) || text === 'stop' || text === '')
        return;
    if (text)
